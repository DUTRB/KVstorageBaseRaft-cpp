...
/**
 * @description: 接收leader发来的日志请求，主要检验用于检查当前日志是否匹配并同步leader的日志到本机
...
/**
 * @description: 实际发送心跳，判断到底是构造需要发送的rpc，并多线程调用sendRequestVote处理rpc及其相应
 * @return {*}
 */
  ...
  if (m_status == Leader) {
    ...
    for (int i = 0; i < m_peers.size(); i++) {
      ...
      std::thread t(&Raft::sendAppendEntries, this, i, appendEntriesArgs, appendEntriesReply,
                    appendNums);  // 创建新线程并执行函数，并传递参数
      t.detach();
    }
    m_lastResetHearBeatTime = now();  // leader发送心跳，重置心跳时间，就不是随机时间了
  }
}

/**
 * @description: 选举超时判断，查看是否发起该选举，如果该发起选举则执行 doElection()
 * @return {*}
 */
...
/**
 * @description: 接收别人发来的选举请求，主要检验是否要给对方投票。
...
/**
 * @description: 负责发送选举中的RPC，在发送完rpc后还需要负责接收并处理对端发送回来的响应。
 * @return {*}
 */
